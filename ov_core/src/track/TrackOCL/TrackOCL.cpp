...
void TrackOCL::feed_new_camera(const CameraData &message) {
    ...
    if ( message.sensor_ids.empty() || 
        (message.sensor_ids.size() != message.images.size() && message.sensor_ids.size() != message.cl_images.size()) || 
        (message.images.size() != message.masks.size() && message.cl_images.size() != message.masks.size()))
    {    
    ...
    size_t num_cl_images = message.cl_images.size();
    ...
    if (num_images == 1) {
    ...
    else if (num_cl_images == 1) {
        feed_monocular_gpu_buf(message, 0);
    } else if (!use_stereo) {
        ...
        for (int i = 0; i < (int)num_cl_images; i++) {
            feed_monocular_gpu_buf(message, i);
        }
    } else {
...
void TrackOCL::feed_monocular_gpu_buf(const CameraData &message, size_t msg_id)
{
    // Lock this data feed for this camera
    size_t cam_id = message.sensor_ids.at(msg_id);
    std::lock_guard<std::mutex> lck(mtx_feeds.at(cam_id));
    ...
    if (pts_last[cam_id].empty()) {
        ...
        perform_detection_monocular_gpu_buf(message.cl_images[cam_id], mask, good_left, good_ids_left, cam_id);
        ...
        ocl_manager.cam_track[cam_id]->build_next_pyramid_gpu_buf(message.cl_images[msg_id]);
    ...
    auto pts_left_old = pts_last[cam_id];
    auto ids_left_old = ids_last[cam_id];
    perform_detection_monocular_gpu_buf(message.cl_images[cam_id], mask, pts_left_old, ids_left_old, cam_id);
    rT3 = boost::posix_time::microsec_clock::local_time();
    ...
    for (size_t i = 0; i < pts_left_new.size(); i++) {
        // Ensure we do not have any bad KLT tracks (i.e., points are negative)
        if (pts_left_new.at(i).pt.x < 0 || pts_left_new.at(i).pt.y < 0 || (int)pts_left_new.at(i).pt.x >= mask.cols ||
            (int)pts_left_new.at(i).pt.y >= mask.rows)
            continue;
        // Check if it is in the mask
        // NOTE: mask has max value of 255 (white) if it should be
        if ((int)message.masks.at(msg_id).at<uint8_t>((int)pts_left_new.at(i).pt.y, (int)pts_left_new.at(i).pt.x) > 127)
...
void TrackOCL::perform_detection_monocular_gpu_buf(const cl_mem img_buf, const cv::Mat &mask0, std::vector<cv::KeyPoint> &pts0,
    ...
    std::vector<cv::KeyPoint> pts0_ext;
    Grider_OCL::perform_griding_gpu_buf(this->ocl_manager.cam_track[id], img_buf, mask0_updated, valid_locs, pts0_ext, num_features, grid_x, grid_y, threshold, true);
